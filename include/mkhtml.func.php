...
function makeChannelById($typeId)
{
	...
	if($TotalResult == 0||strpos($content,'{/seacms:channellist}')===false){
		...
		$tempStr=str_replace("<head>",'<head><script>var seatype="list"; var seaid='.$currentTypeId.';var seapage=1;</script><script src="/'.$GLOBALS['cfg_cmspath'].'js/seajump.js"></script>',$tempStr);
		$content=$tempStr;
...
function makeNewsChannelById($typeId)
{
	...
	if($TotalResult == 0){
		...
		$tempStr = str_replace("{channelpage:page}",1,$tempStr);
		$tempStr =str_replace("<head>",'<head><script>var seatype="newslist"; var seaid='.$currentTypeId.';var seapage=1;</script><script src="/'.$GLOBALS['cfg_cmspath'].'js/seajump.js"></script>',$tempStr);
		$content=$tempStr;
	...
	for($i=$mstart;$i<=$mend;$i++){
		...
		$tmp = str_replace("{channelpage:page}",$i,$tmp);
		$tmp =str_replace("<head>",'<head><script>var seatype="newslist"; var seaid='.$currentTypeId.';var seapage='.$i.';</script><script src="/'.$GLOBALS['cfg_cmspath'].'js/seajump.js"></script>',$tmp);

		$tmp=$mainClassObj->ParseNewsPageList($tmp,$typeIds,$i,$pCount,"newspage",$currentTypeId);
		$tmp=$mainClassObj->parseIf($tmp);
		createTextFile($tmp,sea_ROOT.$channelLink);
