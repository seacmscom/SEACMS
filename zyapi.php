...
//判断是否开启api服务
$vo=explode('#',file_get_contents("data/admin/isapi.txt")); 
$v=$vo[0]; //开关
$o=$vo[1]; if($o=="" OR empty($o)){$o=1;}//排序方式
if($o==1){$orderStr=" order by v_addtime DESC";}
elseif($o==2){$orderStr=" order by v_addtime ASC";}
elseif($o==3){$orderStr=" order by v_id DESC";}
elseif($o==4){$orderStr=" order by v_id ASC";}
else{$orderStr=" order by v_addtime DESC";};

if($v==0){echo "服务已关闭";exit;}
...
function cj()
{
	global $dsql,$rtype,$rpage,$rkey,$rday,$action,$app_apiver,$app_apipagenum,$cfg_basehost,$ids,$orderStr;
	...
	$sql = $sql . $orderStr." limit ".($app_apipagenum * ($rpage-1)).",".$app_apipagenum;
	$dsql->SetQuery($sql);
...
function vlist()
{
	global $dsql,$rtype,$rpage,$rkey,$rday,$action,$app_apiver,$app_apipagenum,$cfg_basehost,$orderStr ;
